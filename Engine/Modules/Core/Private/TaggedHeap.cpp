...
int ws_tagged_heap_init()
{
    ...
    ws_tagged_heap_ptr = (ws_tagged_heap_t*) mmap(0, tagged_heap_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
void* ws_tagged_heap_alloc_block(ws_tag_t tag)
{
    for(size_t i = tagged_heap_reserved_block_count; i < tagged_heap_block_count; i++)
    {
        if(ws_tagged_heap_ptr->block_tags[i] == 0)
        {
            ...
            void* block_address = ws_tagged_heap_ptr + i * tagged_heap_block_size;
    ...
    while(start < end)
    {
        if(tags[start + blocks_found] == 0)
        {
            ...
            if(blocks_found == blocks)
            {
                ...
                void* block_address = ws_tagged_heap_ptr + start * tagged_heap_block_size;
                return block_address;
            }
        }
        else
